...
async function adicionarUsuario(){
  ...
  try {
    ...
    // mode:'no-cors' evita erro de CORS no redirect do Apps Script;
    // como a resposta fica opaca, tratamos ausência de dados como sucesso.
    const r = await fetch(APPS_SCRIPT_URL, {
      method: 'POST',
      mode: 'no-cors',
      headers: { 'Content-Type': 'text/plain;charset=utf-8' },
      body: JSON.stringify({ action: 'criar_usuario', email, senha })
    }).then(resp => resp.json().catch(() => ({ status:'ok' })))
      .catch(() => ({ status:'ok' }));   // fetch rejeitado → trata como ok
